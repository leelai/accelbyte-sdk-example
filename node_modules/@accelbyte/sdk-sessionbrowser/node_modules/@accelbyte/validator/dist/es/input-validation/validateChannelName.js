import isEmpty from 'validator/lib/isEmpty.js';
import matches from 'validator/lib/matches.js';
import { z } from 'zod';
import { CommonValidationErrorType } from './constant/errorType';
import { ValidateLengthErrorType, validateLength } from './validateLength';

const ValidateChannelNameErrorType = z.enum([...ValidateLengthErrorType.options, CommonValidationErrorType.enum.invalidFormat]);
const validateChannelName = (value, { isRequired = true } = {}) => {
  const MAX_LENGTH = 64;
  const REGEX = "^([a-zA-Z0-9]+)((-|_)?[a-zA-Z0-9])*$";
  if (isEmpty(value)) {
    if (!isRequired) {
      return null;
    }
    return ValidateChannelNameErrorType.enum.empty;
  }
  if (!matches(value, REGEX) && value.length <= MAX_LENGTH) {
    return ValidateChannelNameErrorType.enum.invalidFormat;
  }
  return validateLength(value, { max: MAX_LENGTH });
};

export { ValidateChannelNameErrorType, validateChannelName };
//# sourceMappingURL=validateChannelName.js.map
//# sourceMappingURL=validateChannelName.js.map