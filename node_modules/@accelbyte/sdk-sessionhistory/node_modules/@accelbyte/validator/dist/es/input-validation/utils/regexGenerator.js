...
const LetterCaseEnum = z.enum(["lowercase", "uppercase", "mixed", "any"]);
const CharacterLocationEnum = z.enum(["anywhere", "middle"]);
const UNICODE_PATTERN = "^[\\pL\\pN\\pM]*$";
const NUMBER_PATTERN = "0-9";
const LOWERCASE_PATTERN = "a-z";
const UPPERCASE_PATTERN = "A-Z";
const ANYCASE_PATTERN = "A-Za-z";
const DEFAULT_USERNAME_RULE = {
  regex: "()",
...
const DEFAULT_DISPLAYNAME_RULE = {
  regex: "()",
  maxRepeatingAlphaNum: 0,
...
const EMAIL_DOMAIN_REGEX = "^(?:[a-zA-Z0-9-]+\\.)+[a-zA-Z]{2,15}$";
...
const createCharacterSet = (input) => {
  if (!input) {
    return input;
  }
  return `[${input}]`;
};
const createPositiveLookahead = (input) => {
  if (!input) {
    return input;
  }
  return `(?=.*${createCharacterSet(input)})`;
};
const createNonCapturingGroup = (input) => {
  if (!input) {
    return input;
  }
  return `(?:${input})`;
};
const createCapturingGroup = (input) => {
  if (!input) {
    return input;
  }
  return `(${input})`;
};
const createCombination = (pool, length) => {
  let result = [];
...
export { CharacterLocationEnum, DEFAULT_DISPLAYNAME_RULE, DEFAULT_EMAIL_RULE, DEFAULT_PASSWORD_RULE, DEFAULT_USERNAME_RULE, EMAIL_DOMAIN_REGEX, LetterCaseEnum, generatePattern };
